...
new Handle:g_hCvarMaxTraces = INVALID_HANDLE;
new g_iMaxTraces;
...
public OnPluginStart()
{
	...
	g_hCvarMaxTraces = CreateConVar("smac_wallhack_maxtraces", "1280", "Max amount of traces that can be executed in one tick.", FCVAR_PLUGIN, true, 1.0);
	OnMaxTracesChanged(g_hCvarMaxTraces, "", "");
	HookConVarChange(g_hCvarMaxTraces, OnMaxTracesChanged);
...
public Action:Hook_NormalSound(clients[64], &numClients, String:sample[PLATFORM_MAX_PATH], &entity, &channel, &Float:volume, &level, &pitch, &flags)
{
	...
	decl temp;
	if (!entity || !IsValidEdict(entity) || GetTrieValue(g_hIgnoreSounds, sample, temp))
...
SendRadarTeam(team)
{
	...
	for (new i = 0; i < numClients && count < MAX_RADAR_CLIENTS; i++)
	{
		client = iClients[i];
